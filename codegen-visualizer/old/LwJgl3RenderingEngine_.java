 ...
 */
public abstract class LwJgl3RenderingEngine_ extends RenderingEngine {
    ...
    private final String vertexSrc =
            ...
            varying vec3 vColour;void main(void) {
                vColour = colour;    
    ...
    private final String fragmentSrc =
            """
            #version 330
            varying vec3 vColour;layout(location = 0) out vec4 colourOut;
            void main(void) {
    ...
    public LwJgl3RenderingEngine_(/*String lwJglNativePath, */Rendering rendering) {
    ...
    public void startEngine() throws IllegalStateException {
        ...
        // detect the appropriate monitor for fullscreen only
        //long monitor = glfwGetPrimaryMonitor();

        // monitor is used as an argument in glfwCreateWindow, than app 
        // starts in fullscreen mode with the given resolution
        ...
        glfwSetKeyCallback(window, (keyCallback = new GLFWKeyCallback() {
            ...
            public void invoke(long window, int key, int scancode, int action, int mods) {
                ...
                //if (Keyboard.isKeyDown(Keyboard.KEY_F3)) {
                    //Start recording
                    if (recorder == null) {
                        recorder = new GIFRecorder(LwJgl3RenderingEngine_.this);
        ...
        // GL-Version = 4.6 (Core Profile) Mesa 25.0.7-0ubuntu0.24.04.1
        System.out.println("GL-Version    = "+GL11.glGetString(GL_VERSION));
        ...
        // glfwMakeContextCurrent() and GL.createCapabilities() must be invoked 
        // before
        
        // glShadeModel is part of the fixed-function pipeline (used for flat vs 
        // smooth shading in legacy OpenGL).
        // Control shading in the vertex/fragment shaders instead
    ...
    private int axesVao;
    private int axesShaderProgram;
    
    void renderAxes(){
        glUseProgram(axesShaderProgram);
        glBindVertexArray(axesVao);

        // Draw 6 vertices (3 lines → 2 vertices each)
     ...
     * appropriate state. 
     */
    public void initGL() throws IllegalStateException {
        
        // init visualisation of the coordinate axes
        
        axesVao = createAxesVAO();
        axesShaderProgram = createShaderProgram(axes_vertex_shader, axes_fragment_shader);
        
        
        // init shader based transformation between model and projection
        
        program = createShaderProgram(vertexSrc, fragmentSrc);               
        ...
        // preparation of point cloud visualisation
        
        // create an empty array of vbos
        vao = glGenVertexArrays();
        
        // Einen oder mehrere eindeutige Namen für Pufferobjekte generieren. Diese 
        // Namen sind wie Handle, die später verwendet werden, um Daten wie 
        // Vertexinformationen, Texturdaten oder andere Datenarrays für die GPU 
        // bereitzuhalten und zu nutzen
        //vbo = glGenBuffers();
        // Pufferobjekt an einen bestimmten Bindungspunkt (Target) binden
    ...
    public void run() {
        ...
        try {
            ...
            // is no need to call this function, as it is called by glfwInit before 
            // it returns failure.
            glfwTerminate();
    ...
    /**
     * aus pong --> code soll in meine run-Methode
     * TODO
     * 
     * Clears the screen and renders all scene objects.
     */
    public void render() {
        // glClear(GL_COLOR_BUFFER_BIT); // pong
        GL11.glClear(GL11.GL_COLOR_BUFFER_BIT | GL11.GL_DEPTH_BUFFER_BIT); // clear the screen

        renderAxes();
        
        glUseProgram(program); // was genau macht dieses Programm?
        // binds the vertex array object with name array. array is the name of 
        // a vertex array object previously returned from a call to glGenVertexArrays, 
        // or zero to break the existing vertex array object binding. If no 
        // vertex array object with name array exists, one is created when array 
        // is first bound.
        glBindVertexArray(vao); // was ist in vao drin?
        
        // set the value of a uniform variable of type mat4x4 (a 4x4 matrix) within 
        // a shader program. It takes the uniform's location, the number of matrices 
        // to update(?), a boolean indicating if the data should be transposed, 
        // and a pointer to an array of floating-point values representing the matrix data
        glUniformMatrix4fv(projectionLoc, false, projectionMatrix);
        
        
        // mein spezifischer point cloud rendering code
        //TODO überprüfen ob das so geht
        ...
        //TODO
        // das geht so nicht mehr! Wie macht das Pong?
        // dafür gibts jetzt den shader siehe oben
    ...
    /*private void changeSize(float w, float h) {
        ...
        // das gibt es so auch nicht mehr
        GL11.glMatrixMode(GL11.GL_PROJECTION);
        GL11.glLoadIdentity();
